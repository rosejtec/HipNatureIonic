...
export class PaymentModalPage implements OnInit {
  ...
  constructor(private creditPlanService: CreditplanService, private purchaseplanservice: PurchaseplanService, private creditCardservice: CreditcardService, private modalCtrl: ModalController, private alertCtrl: AlertController, private planService: PlanService) {
  ...
  makePayment(value) {
    ...
    for (let [index, p] of this.creditcards.entries()) {
      if (p.creditCardId == this.creditCardChosen && p.cvv == this.creditCardChosenCCV.toString()) {
        console.log("Pass")
        console.log(value['sessionLimit'])
        if (value['sessionLimit'] == null) {
          console.log("do credit plan payment")
          this.creditPlanService.makePurchaseCreditPlan(this.creditCardChosen, value).subscribe(response => {
